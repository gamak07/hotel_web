  ...
  return (
    <div className="bg-[#ffffff] h-[60vh] w-[100%] overflow-y-auto">
      {bookedRoom.map((room) => (
        <div
          key={room.id}
          className="flex h-full w-full mx-auto p-6 bg-white shadow-lg rounded-lg"
        >
          <div className="w-[60%]">
            <img
              src={room.image}
          ...
          <div className="w-[40%] p-[1rem]">
            <div>
              ...
              <p className="text-gray-700">
                <strong>Check In:</strong> {room.formattedCheckInDate}
              </p>
              <p className="text-gray-700">
                <strong>Check Out:</strong> {room.formattedCheckOutDate}
              </p>
              <p className="text-gray-700">
                <strong>Price per Night:</strong> ${room.pricePerNight}
              </p>
              <p className="text-gray-700">
                <strong>Total Days:</strong> {Math.ceil(room.totalDays)}
              </p>
              <p className="flex items-center justify-between text-lg font-semibold text-gray-800 mt-4">
                <span className="font-bold">Total Price:</span> $
                {(room.pricePerNight * room.totalDays).toFixed(2)}
              </p>
            </div>
            <div className="flex justify-between mt-4">
              <Button onClick={() => cancelBooking(room.id)}>
                <p className="bg-red-600 text-white font-bold py-2 px-4 rounded hover:bg-red-700 transition duration-200">
                  Cancel Booking
                </p>
              </Button>
              <Button
              ...
              >
                <p className="bg-blue-600 text-white font-bold py-2 px-4 rounded hover:bg-blue-700 transition  duration-200">
                  Checkout
                </p>
              </Button>
            </div>
